...
export async function processReturnExchangeAction(id: number, payload: any) {
  const result = await salesServerService.processReturn(id, payload);
  revalidatePath("/sales-orders");
  revalidatePath(`/sales-orders/${id}`);
  return result ? JSON.parse(JSON.stringify(result)) : null;
}

export async function returnExchangeAction(
  id: number,
  data: {
